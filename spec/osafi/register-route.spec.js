...
describe('register-route', () => {
  ...
  pluginTest(
    pluginPath,
    'POSTs the room link to a director with authorization',
    async ({ room }) => {
  ...
  pluginTest(
    pluginPath,
    'Logs error when response from director is not 2xx',
    async ({ room }) => {
      nock('http://localhost:8080').post('/routes').reply(400, 'something bad happened');
